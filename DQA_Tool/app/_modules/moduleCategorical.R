...
moduleCategoricalServer <- function(input, output, session, rv, input_re){
  ...
  observe({
    if (isFALSE(rv$target_getdata) && isFALSE(rv$source_getdata)){
      ...
      output$cat_selection_uiout <- renderUI({
        selectInput(session$ns("categorical_sel"), "Select variable", rv$dqa_categorical, multiple=FALSE, selectize=FALSE, size = 10)
      })
      
      # calculate rv$dqa_categorical_results
      # list-object-structure:
      # rv$dqa_categorical_results
      ## description
      ### variable
      #### source_data
      #### target_data
      ...
      if (is.null(rv$dqa_categorical_results$description)){
        withProgress(message = "Populating descriptions of categorical variables", value = 0, {
          
          for (i in names(rv$dqa_categorical)){
            incProgress(1/length(rv$dqa_categorical), detail = paste("... working at description of", i, "..."))
            # generate descriptions
            desc_dat <- rv$mdr[dqa_assessment==1,][grepl("^dt\\.", key),][variable_name==rv$dqa_categorical[[i]],.(source_system, source_variable_name, source_table_name, fhir)]
            if (nrow(desc_dat)>1){
              rv$dqa_categorical_results$description[[rv$dqa_categorical[[i]]]]$source_data <- list(var_name = desc_dat[source_system=="csv", source_variable_name],
                                                                                                table_name = desc_dat[source_system=="csv", source_table_name],
                                                                                                fhir_name = desc_dat[source_system=="csv", fhir])
              
              rv$dqa_categorical_results$description[[rv$dqa_categorical[[i]]]]$target_data <- list(var_name = desc_dat[source_system==rv$target_db, source_variable_name],
      ...
      if (is.null(rv$dqa_categorical_results$counts)){
      ...
      if (is.null(rv$dqa_categorical_results$statistics)){
      ...
      observeEvent(input_re()[["moduleCategorical-categorical_sel"]], {
        ...
        stat_out <- rv$dqa_categorical_results$statistics[[input_re()[["moduleCategorical-categorical_sel"]]]]
        
        
        
        # render source description
        output$cat_selection_description_source <- renderTable({
        ...
        output$cat_selection_description_target <- renderTable({
        ...
        output$cat_selection_counts_source <- renderTable({
          tryCatch({
        ...
        output$cat_selection_counts_target <- renderTable({
          tryCatch({
        ...
        output$cat_selection_source_table <- renderTable({
...
moduleCategoricalUI <- function(id){
  ns <- NS(id)
  ...
  tagList(
    fluidRow(
      column(4,
             box(title = "Select variable",
                 uiOutput(ns("cat_selection_uiout")),
                 width = 12
             )
      ),
      column(8,
             box(title="Results Source Data",
                 ...
                 fluidRow(
                   column(6,
                          ...
                          tableOutput(ns("cat_selection_description_source"))
                   ),
                   column(6,
                          h5(tags$b("Variable Description")),
                          tableOutput(ns("cat_selection_counts_source"))
                   )
                 )
             ),
             box(title="Results Target Data",
                 ...
                 fluidRow(
                   column(6,
                          h5(tags$b("Source Format")),
                          tableOutput(ns("cat_selection_description_target"))
                   ),
                   column(6,
                          h5(tags$b("Variable Description")),
                          tableOutput(ns("cat_selection_counts_target"))
                   )
                 )
             )
           )
    ...
    fluidRow(
      box(title="Statistics Source",
          tableOutput(ns("cat_selection_source_table")),
          width=6
      ),
      box(title="Statistics Target",
          tableOutput(ns("cat_selection_target_table")),
          width=6
      )
    )
  )
