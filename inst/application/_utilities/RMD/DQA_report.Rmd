...
```{r results='asis'}
DQAstats::renderDataMap(rv$datamap$target_data)
```

## Source Data System

```{r results='asis'}
DQAstats::renderDataMap(rv$datamap$source_data)
```
...
```{r results='asis'}
DQAstats::kableTable(rv$checks$etl)
```
...
```{r results='asis'}
DQAstats::kableTable(rv$completeness[,c(1,2,4)])
```

## Relative  

```{r results='asis'}
DQAstats::kableTable(rv$completeness[,c(1,3,5)])
```
...
```{r results='asis'}
DQAstats::kableTable(rv$checks$value_conformance)
```
...
```{r results='asis'}
DQAstats::renderResults(rv$results_descriptive, rv$conformance$value_conformance)
```
...
```{r results='asis'}
# plausi results
DQAstats::renderUniqPlausis(rv$results_plausibility_uniqueness)
```
...
```{r results='asis'}
# plausi results
DQAstats::renderAtempPlausis(rv$results_plausibility_atemporal, rv$conformance$value_conformance)
```
