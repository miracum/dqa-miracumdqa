...
select_vars <- function(mdr_use) {
  sel_vars <- sapply(mdr.use[,get("source_variable_name")], function(x){paste0("DATA ->> '", x, "'", "\tAS\t\"", mdr.use[source_variable_name==x,variable_name], "\"")})
...
mdr <- mdr[source_system_name=="fhir",]
...
# FROM
#   fhirmiracum.", mdr.use[source_variable_name==looplist[[i]]$var2,source_table_name], ") AS a ON
...
writeLines(jsonlist, "./inst/application/_utilities/SQL/SQL_fhir.JSON")
