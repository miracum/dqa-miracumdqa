...
loadSourcePlausibilities <- function(plausi, rv, headless=FALSE){
  ...
  if (plausi == "pl.item02_source"){
    # item02
    outlist <- merge(rv$list_source[["ICD.CSV"]][grepl("O[0-9]", get("condition_code_coding_code")),c("condition_encounter_identifier_value", "condition_code_coding_code"), with=F],
                     rv$list_source[["FALL.CSV"]][,c("encounter_identifier_value", "patient_gender", "patient_identifier_value"), with=F],
    ...
    # item03
    outlist <- merge(rv$list_source[["ICD.CSV"]][grepl("C5[1-8]", get("condition_code_coding_code")),c("condition_encounter_identifier_value", "condition_code_coding_code"), with=F],
                     rv$list_source[["FALL.CSV"]][,c("encounter_identifier_value", "patient_gender", "patient_identifier_value"), with=F],
                     by.x = "condition_encounter_identifier_value",
                     by.y = "encounter_identifier_value",
                     all.x = TRUE)
    ...
    # item04
    outlist <- merge(rv$list_source[["ICD.CSV"]][grepl("C6[0-3]", get("condition_code_coding_code")),c("condition_encounter_identifier_value", "condition_code_coding_code"), with=F],
                     rv$list_source[["FALL.CSV"]][,c("encounter_identifier_value", "patient_gender", "patient_identifier_value"), with=F],
                     by.x = "condition_encounter_identifier_value",
                     by.y = "encounter_identifier_value",
                     all.x = TRUE)
    ...
    # item05
    outlist <- rv$list_source[["FALL.CSV"]][grepl("05xx", get("encounter_hospitalization_class")),c("encounter_identifier_value",
                                                                           "patient_gender",
...
calcPlausiDescription <- function(desc_dat, rv, sourcesystem){
  if (nrow(desc_dat)>1){
    ...
    description$target_data <- list(var_name = desc_dat[get("source_system")==rv$target_db, get("source_variable_name")],
                                    ...
                                    sql_from = desc_dat[get("source_system")==rv$target_db, get("sql_from")],
                                    sql_join_table = desc_dat[get("source_system")==rv$target_db, get("sql_join_table")],
                                    sql_join_type = desc_dat[get("source_system")==rv$target_db, get("sql_join_type")],
                                    sql_join_on = desc_dat[get("source_system")==rv$target_db, get("sql_join_on")],
                                    sql_where = desc_dat[get("source_system")==rv$target_db, get("sql_where")])
    return(description)
  } else {
    return(NULL)
  }
