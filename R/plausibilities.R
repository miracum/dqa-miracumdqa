...
loadSourcePlausibilities <- function(plausi, rv, headless=FALSE){
  ...
  if (plausi == "pl.item02_source"){
    # item02
    outlist <- merge(rv$list_source[["ICD.CSV"]][grepl("O[0-9]", data.table::get("condition_code_coding_code")),c("condition_encounter_identifier_value", "condition_code_coding_code"), with=F],
    ...
    # item03
    outlist <- merge(rv$list_source[["ICD.CSV"]][grepl("C5[1-8]", data.table::get("condition_code_coding_code")),c("condition_encounter_identifier_value", "condition_code_coding_code"), with=F],
                     rv$list_source[["FALL.CSV"]][,c("encounter_identifier_value", "patient_gender", "patient_identifier_value"), with=F],
                     by.x = "condition_encounter_identifier_value",
                     by.y = "encounter_identifier_value",
                     all.x = TRUE)
    ...
    # item04
    outlist <- merge(rv$list_source[["ICD.CSV"]][grepl("C6[0-3]", data.table::get("condition_code_coding_code")),c("condition_encounter_identifier_value", "condition_code_coding_code"), with=F],
                     rv$list_source[["FALL.CSV"]][,c("encounter_identifier_value", "patient_gender", "patient_identifier_value"), with=F],
                     by.x = "condition_encounter_identifier_value",
                     by.y = "encounter_identifier_value",
                     all.x = TRUE)
    ...
    # item05
    outlist <- rv$list_source[["FALL.CSV"]][grepl("05xx", data.table::get("encounter_hospitalization_class")),c("encounter_identifier_value",
                                                                           "patient_gender",
