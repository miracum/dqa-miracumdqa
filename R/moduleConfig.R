...
moduleConfigServer <- function(input, output, session, rv, input_re){
  ...
  observeEvent(input_re()[["moduleConfig-config_sourcedir_in"]], {
    rv$sourcefiledir <- shinyFiles::parseDirPath(roots, input_re()[["moduleConfig-config_sourcedir_in"]])
  })
  ...
  observeEvent(input_re()[["moduleConfig-config_targetdb_rad"]], {
    ...
    shiny::updateTextInput(session, "config_targetdb_host", value = rv$db_settings$host)
    shiny::updateTextInput(session, "config_targetdb_port", value = rv$db_settings$port)
    shiny::updateTextInput(session, "config_targetdb_user", value = rv$db_settings$user)
    shiny::updateTextInput(session, "config_targetdb_password", value = rv$db_settings$password)
  })
  ...
  observe({
    ...
    if (is.null(rv$sql)){
      if (input_re()[["moduleConfig-config_targetdb_rad"]] == "i2b2"){
        cat("\nLoading i2b2 SQLs\n")
        rv$sql <- jsonlite::fromJSON("./_utilities/SQL/SQL_i2b2.JSON")
      } else if (input_re()[["moduleConfig-config_targetdb_rad"]] == "omop"){
        cat("\nLoading omop SQLs\n")
        rv$sql <- jsonlite::fromJSON("./_utilities/SQL/SQL_omop.JSON")
      }
    }
  })
  ...
  observe({
    if (is.null(rv$sitenames)){
      rv$sitenames <- jsonlite::fromJSON("./_utilities/MISC/sitenames.JSON")
